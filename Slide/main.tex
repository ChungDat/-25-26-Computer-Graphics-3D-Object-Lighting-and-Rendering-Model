...
\begin{frame}{Bối cảnh}
	...
	Mục tiêu của đồ hoạ máy tính là nhằm \textbf{mô phỏng thế giới thực vào không gian hai chiều trên màn hình} và có thể giao tiếp với người dùng thông qua các thiết bị tương tác. Nền tảng của lĩnh vực này là hai khái niệm cốt lõi: mô hình chiếu sáng (Lighting Models) và kết xuất đối tượng (Object Rendering).
\end{frame}
...
\begin{frame}{Động lực nghiên cứu}
	\textbf{Giải trí và truyền thông:} Trong ngành công nghiệp điện ảnh và trò chơi điện tử, cải thiện trải nghiệm đắm chìm (Immersive Experience) của người dùng là mục tiêu của mọi công ty nhằm thu hút khách hàng mới. Các trò chơi có thể hiển thị hàng triệu đa giác mỗi giây, và các kỹ xảo được sử dụng trong phim điện ảnh đã quá thực tế và không thể dễ dàng phân biệt với các tài nguyên được tạo dựng thủ công \cite{principles_1990}. Các kỹ thuật kết xuất tiên tiến, đặc biệt là dò tia thời gian thực (Real-time Ray Tracing), cho phép tạo ra các hiệu ứng phản xạ, bóng đổ, và chiếu sáng toàn cục phức tạp, mang lại trải nghiệm chân thực và hấp dẫn hơn.
\end{frame}
...
\begin{frame}{Phát biểu bài toán}
	Bài toán tổng hợp hình ảnh trong đồ họa máy tính là quá trình biến đổi một mô tả toán học trừu tượng của một cảnh ba chiều thành một hình ảnh hai chiều quang thực. Mục tiêu cuối cùng là làm cho hình ảnh 2 chiều đầu ra trở nên không thể phân biệt được (hoặc càng giống càng tốt) với một bức ảnh chụp của một cảnh tương tự trong thế giới thực. Quá trình này có thể được phân tách thành hai bài toán con: định nghĩa Mô hình Chiếu sáng và thực thi quá trình Kết xuất Đối tượng.
\end{frame}
...
\begin{frame}{Framework bài toán}
	\begin{enumerate}
		...
		\begin{itemize}
			...
			\item \textbf{Biến đổi Hình học (Geometric Transformation):} Sắp xếp tất cả các đối tượng và camera vào một hệ quy chiếu chung. Điều này bao gồm các thao tác di chuyển, xoay, và thay đổi kích thước các đối tượng từ không gian mô hình riêng của chúng vào một không gian thế giới (world space) thống nhất.
		\end{itemize}
...
\begin{frame}{Framework bài toán}
	\begin{enumerate}
		\item \textbf{Tính toán Chiếu sáng và Tô bóng (Lighting and Shading Calculation):} Đây là công đoạn Mô hình Chiếu sáng được áp dụng. Sau khi xác định được bề mặt nhìn thấy tại mỗi pixel, hệ thống sẽ tính toán màu sắc của điểm đó. Kết quả là giá trị màu cuối cùng cho pixel đó.
...
\begin{frame}{Nguồn sáng hình học}
	...
	Nguồn sáng điểm được đặt trong cảnh và chiếu sáng theo mọi hướng, mô phỏng nguồn sáng như bóng đèn điện \cite{principles_1990}.\\
	\begin{center}
...
\begin{frame}{Nguồn sáng phi hình học}
	...
	Nguồn sáng môi trường là một lượng ánh sáng được chiếu lên tất cả bề mặt, không phụ thuộc vào hướng và khoảng cách. Nguồn sáng môi trường giúp loại bỏ các vùng "thuần tối" (pure black) ở các bề mặt hướng ra xa nguồn sáng \cite{principles_1990}.
\end{frame}

\begin{frame}{Nguồn sáng phi hình học}
	\textbf{Nguồn sáng phát xạ - Emissive Light}\\
	Nguồn sáng phát xạ được sử dụng để mô tả các bề mặt có khả năng tự phát sáng nhưng không phải một nguồn sáng đúng nghĩa, cho phép bề mặt "phản xạ" ánh sáng ảo dù không tồn tại nguồn sáng trong cảnh \cite{principles_1990}, mô phỏng các vật thể tự phát sáng.
...
\begin{frame}{Cài đặt}
	\begin{itemize}
		...
		\item GLAD: Thư viện tải hàm OpenGL (OpenGL function loader). GLAD đảm bảo chương trình có thể sử dụng đúng các hàm OpenGL mà driver GPU hỗ trợ.
		\item GLM: Thư viện toán học cho đồ họa máy tính, được thiết kế theo chuẩn của GLSL. GLM cung cấp các kiểu dữ liệu và phép toán cho vector, ma trận, quaternion và các phép biến đổi hình học thường dùng trong đồ họa 3D.
	\end{itemize}
\end{frame}
	...
	\begin{columns}[T]
		...
		\begin{column}{0.5\textwidth}
			\begin{center}
				...
				\begin{itemize}
					\item initWindow()
					\item gladLoadGLLoader()
					\item glEnable(GL\_DEPTH\_TEST)
					\item ImGui::CreateContext()
				\end{itemize}
			\end{center}
		\end{column}
		...
		\begin{column}{0.5\textwidth}
			\begin{center}
				Lưu trữ đối tượng
				\begin{itemize}
					\item glGenVertexArrays(), glGenBuffers(), Assimp::Importer.ReadFile()
					\item glDrawArrays(), glDrawElements()
					\item createObject()
				\end{itemize}
			\end{center}
		\end{column}
	...
	\begin{columns}[T]
		\begin{column}{0.5\textwidth}
			\begin{center}
				...
				\begin{itemize}
					\item glm::scale(), glm::rotate(), glm::translate()
					\item glm::lookAt()
				\end{itemize}
			\end{center}
		\end{column}
		...
		\begin{column}{0.5\textwidth}
			\begin{center}
				Lưu trữ Shader và Texture
				\begin{itemize}
					\item glGenTextures(), glBindTexture();
					\item glCreateShader(), glShaderSource(), glCompileShader()
				\end{itemize}
			\end{center}
		\end{column}
	\end{columns}
