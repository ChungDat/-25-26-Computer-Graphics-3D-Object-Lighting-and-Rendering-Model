...
uniform float shadowBias; // single bias
...
vec3 CalcDirLight(DirectionalLight light, sampler2D shadowMap, mat4 lightSpaceMatrix, vec3 normal, vec3 viewDir) {
	...
	// shadow
	float shadow = ShadowCalculation(shadowMap, lightSpaceMatrix * vec4(fragPos, 1.0f));
...
vec3 CalcSpotLight(SpotLight light, sampler2D shadowMap, mat4 lightSpaceMatrix, vec3 normal, vec3 viewDir) {
	...
	// shadow
	float shadow = ShadowCalculation(shadowMap, lightSpaceMatrix * vec4(fragPos, 1.0));
	float shadowStrength = 0.7;
